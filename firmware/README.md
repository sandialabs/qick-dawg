...
* Available build file are for the 4x2, zcu111 and zcu216. Go the board you want to build and into the `/src/` directory. You will find a project script (`proj_v2.tcl`) and a block design script (`bd_2022-1_2.tcl` or similar version).
* Install the version of Vivado specified by the block design filename (older or newer versions will fail!). Start Vivado.
* In the Tcl console at the bottom of the screen navigate to this directory of the `'src` then run,
```
 source ./proj_2.tcl
 ```
This will create the block diagram design of the of the firmware. Do not run `bd_2022-1_2.tcl`.  

* After the block diagram is built, select "Generate Bitstream"  on the navigation menu at the left. This will compile the firmware.
